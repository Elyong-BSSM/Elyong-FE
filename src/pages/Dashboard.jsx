...
const Dashboard = () => {
  ...
  const [alarms, setAlarms] = useState([
    {
      ...
      minEyeSize: 50,
      goalScore: 80,
      scoreDropRate: 30 // 기존 데이터에도 추가
    },
    {
      id: 2,
      time: "07:00",
      date: "1111111", // 매일
      enabled: false,  // OFF 상태
      audio: "army",
      minEyeSize: 50,
      goalScore: 50,
      scoreDropRate: 50 // 기존 데이터에도 추가
    }
  ]);

  // 폼 데이터 (입력값) -> scoreDropRate 추가됨
  const [formData, setFormData] = useState({
    ...
    minEyeSize: 50,
    goalScore: 50,
    scoreDropRate: 50 // ⭐ 추가된 필드: 점수 하락률 (기본값 50)
  });
  ...
  const handleSelectAlarm = (alarm) => {
    ...
    // 선택한 알람의 데이터로 폼 채우기 (scoreDropRate 포함)
    setFormData({
      time: alarm.time,
      days: daysArray,
      ...
      scoreDropRate: alarm.scoreDropRate || 50
    });
  };
        ...
        <div style={styles.rightPanel}>
          {!isEditing ? (
            ...
            <div style={styles.formContainer}>
              ...
              <div style={styles.formGroup}>
                <div style={styles.labelRow}>
                  <label style={styles.label}>점수 하락률 ({formData.scoreDropRate})</label>
                </div>
                <input 
                  type="range" min="0" max="100" // 범위는 필요에 따라 조절 가능
                  value={formData.scoreDropRate} 
                  onChange={(e) => handleChange('scoreDropRate', e.target.value)} 
                  style={styles.slider}
                />
              </div>
...
const styles = {
  ...
  // 왼쪽 패널: alignItems: 'center'가 이미 가로 중앙 정렬을 담당함
  ...
  // (+) 버튼 컨테이너: 확실한 중앙 정렬을 위해 flex 속성 추가 (선택 사항이지만 안전책)
  plusContainer: { marginBottom: '30px', display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' },
  ...
  alarmCard: { width: '100%', maxWidth: '450px', backgroundColor: '#252525', padding: '25px 30px', borderRadius: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', boxShadow: '0 4px 20px rgba(0,0,0,0.2)', transition: '0.3s' },
  ...
  toggleTrack: { width: '50px', height: '26px', borderRadius: '15px', position: 'relative', cursor: 'pointer', transition: 'background-color 0.3s ease' },
  toggleKnob: { width: '20px', height: '20px', backgroundColor: '#fff', borderRadius: '50%', position: 'absolute', top: '3px', left: '3px', transition: 'transform 0.3s cubic-bezier(0.4, 0.0, 0.2, 1)' },

  // 오른쪽 패널
  rightPanel: { flex: 1, backgroundColor: '#252525', padding: '40px', borderLeft: '1px solid #333', overflowY: 'auto' },
  emptyState: { height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#555', textAlign: 'center' },
  formContainer: { display: 'flex', flexDirection: 'column', gap: '25px' }, // 간격 약간 조정
